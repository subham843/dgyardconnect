    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Service area center set from current location.'),
          ),
        );
      }
    } catch (e) {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (widget.fromPhone) {
        ...
        if (user == null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Session expired. Please start again from phone number.',
              ),
            ),
          );
          return;
      ...
      if (!FirestoreService.isAvailable) {
        ...
        context.go(
          ...
          extra: {
            ...
            'nextRoute': RouteNames.technicianHome,
            'extra': null,
          },
        );
        return;
      ...
      final profile = <String, dynamic>{
        ...
        'phone': widget.fromPhone
            ? (AuthService().currentUser?.phoneNumber ?? '')
            : '',
      };
      if (widget.fromPhone && widget.initialServiceArea != null) {
        profile['buildingApartment'] =
            widget.initialServiceArea!.buildingApartment;
        profile['houseFlatShopNumber'] =
            widget.initialServiceArea!.houseFlatShopNumber;
        profile['landmark'] = widget.initialServiceArea!.landmark;
      }
      await FirestoreService.users().doc(uid).set({
        'role': 'technician',
        'technicianLevel': 'gold',
        'approved': false,
        'online': false,
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Select skills and service area. You will be notified once approved.',
                        style: GoogleFonts.plusJakartaSans(
                          fontSize: 14,
                      ...
                      if (FirestoreService.isAvailable)
                        CategorySubcategorySkillsPicker(
                          ...
                          onSkillsChanged: (ids) => setState(() {
                            _selectedSkillIds.clear();
                            _selectedSkillIds.addAll(ids);
                          }),
                        ),
                      if (FirestoreService.isAvailable)
                        const SizedBox(height: 16),
                      if (!widget.fromPhone &&
                          widget.initialServiceArea == null)
        ...
        children: [
          ...
          SizedBox(
            ...
            child: TextFormField(
              ...
              onChanged: (v) =>
                  setState(() => _serviceRadiusKm = double.tryParse(v) ?? 25),
            ),
          ),
        ],
      ),
