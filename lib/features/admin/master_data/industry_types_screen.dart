...
class IndustryTypesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (!FirestoreService.isAvailable) {
      return Scaffold(
        appBar: AppBar(title: const Text('Industry types')),
        body: const Center(child: Text('Firebase is not configured.')),
      );
    }
    return Scaffold(
      appBar: AppBar(
        title: const Text('Industry types'),
        leading: IconButton(
      ...
      builder: (ctx) => Padding(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(id == null ? 'Add industry type' : 'Edit industry type', style: Theme.of(ctx).textTheme.titleLarge),
              ...
              FilledButton(
                onPressed: () async {
                  ...
                  if (id == null) {
                    await FirestoreService.industryTypes().add({'name': name});
                  } else {
                    await FirestoreService.industryTypes().doc(id).update({'name': name});
                  }
  ...
  Future<void> _delete(BuildContext context, String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Delete industry type?'),
        actions: [
    ...
    if (ok == true) await FirestoreService.industryTypes().doc(id).delete();
  }
}
