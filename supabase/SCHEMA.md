...
**Voice multi-provider:** Tenant secrets under `api_secrets.voice.<provider>` (`exotel` | `twilio` | `telnyx` | `plivo` | `vonage` | `knowlarity` | `myoperator`). Active provider in `api_config.voice.provider`. `bos-voice-dial`: Exotel/Twilio/Telnyx/Plivo/Knowlarity/MyOperator live; Vonage RS256 JWT from tenant private key (`_shared/vonage_jwt.ts`). **Telnyx:** dial with `record=record-from-answer` + webhook; outbound `call.answered` → Call Control `speak`; inbound `call.initiated` → answer + greeting + lead; hangup waits ~45s for `call.recording.saved` then completes (Flutter `reconcilePendingVoiceCompletions` backup). Settings: masked keys + Test dial + Verify + webhook copy + TTS preview. Hub shows voice provider health. Soft-delete via `bos_voice_calls.deleted_at`. Reports: voice live/stub/STT/inbound/avg duration.
