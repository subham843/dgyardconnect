...
**Voice multi-provider:** Tenant secrets under `api_secrets.voice.<provider>` (`exotel` | `twilio` | `telnyx` | `plivo` | `vonage` | `knowlarity` | `myoperator`). Active provider in `api_config.voice.provider`. Config: `inbound_greeting`, `auto_callback_missed`. Telnyx optional `public_key` for Ed25519 webhook verify. `bos-voice-dial` + webhook speak/record/inbound answer; missed inbound queues outbound callback (~5 min). Hangup waits ~45s for recording then completes. Hub voice health; Billing voice minutes meter vs plan limit. Soft-delete via `bos_voice_calls.deleted_at`.
