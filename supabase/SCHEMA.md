...
**Phase D CRM + harden + RAG:** Tasks/calendar (`bos_activities.due_at` UI). Configurable deal stages (`bos_deals.stage` text + stage CRUD). Lead merge RPC `bos_merge_leads`. Attachments `bos_attachments` + storage bucket `bos-attachments` (Flutter file upload + URL link). Plan limits `bos_assert_lead_limit` / `bos_assert_user_limit` + Edge `assertUsageLimit` + Billing upgrade CTA. Invite email `bos-invite-email` (sent/stub feedback in UI). KB embeddings on chunks + vector/keyword retrieve in `bos-ai-reply`. Voice dial `bos-voice-dial` (Exotel/Twilio when tenant secrets set; else stub).
