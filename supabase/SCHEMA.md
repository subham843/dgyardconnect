...
**Voice multi-provider:** Tenant secrets under `api_secrets.voice.<provider>` (`exotel` | `twilio` | `telnyx` | `plivo` | `vonage` | `knowlarity` | `myoperator`). Active provider in `api_config.voice.provider`. Config: `inbound_greeting`, `auto_callback_missed`. Telnyx optional `public_key` for Ed25519 webhook verify. Missed inbound queues callback unless `bos_opt_outs` / lead `meta.do_not_call`. Hub/Voice **Run due** dials scheduled callbacks. Settings **Voice readiness** checklist. Soft-delete via `bos_voice_calls.deleted_at`.
