...
**Voice multi-provider:** Tenant secrets under `api_secrets.voice.<provider>` (`exotel` | `twilio` | `telnyx` | `plivo` | `vonage` | `knowlarity` | `myoperator`). Active provider in `api_config.voice.provider`. Config: `inbound_greeting`, `auto_callback_missed`, `sarvam_speaker` / `sarvam_model`. **Twilio conversational dial:** `bos-voice-dial` uses Call `Url` → `bos-voice-twiml` (Sarvam TTS `<Play>` + `<Gather>` listen → AI reply in customer language → loop). Audio clips in public Storage bucket `bos-voice-audio`. Telnyx optional `public_key` for Ed25519 webhook verify. Missed inbound queues callback unless `bos_opt_outs` / lead `meta.do_not_call` (Lead detail toggle). Campaign `trigger_voice` queues `bos_voice_calls` with `scheduled_at` (Run due). Inbox **Queue AI call**. Hub/Voice **Run due**. Settings readiness + webhook event dogfood/test ping. Soft-delete via `bos_voice_calls.deleted_at`.

**Phase D CRM + harden + RAG:** Tasks/calendar (`bos_activities.due_at` UI). Configurable deal stages (`bos_deals.stage` text + stage CRUD). Lead merge RPC `bos_merge_leads`. Attachments `bos_attachments` + storage bucket `bos-attachments` (Flutter file upload + URL link). Plan limits `bos_assert_lead_limit` / `bos_assert_user_limit` + Edge `assertUsageLimit` + Billing upgrade CTA. Invite email `bos-invite-email` (sent/stub feedback in UI). KB embeddings on chunks + vector/keyword retrieve in `bos-ai-reply` with **citations** in response/Inbox/public chat. Voice dial `bos-voice-dial` (Exotel/Twilio/Telnyx/… when tenant secrets set; else stub). Campaign run marks live API failures as `failed`; optional voice auto-dial. Reports: usage 30d + delivery breakdown + Super Admin block.
