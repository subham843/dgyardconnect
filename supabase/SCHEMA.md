...
**Voice multi-provider:** Tenant secrets under `api_secrets.voice.<provider>` (`exotel` | `twilio` | `telnyx` | `plivo` | `vonage` | `knowlarity` | `myoperator`). Active provider in `api_config.voice.provider`. `bos-voice-dial`: Exotel/Twilio/Telnyx/Plivo/Knowlarity/MyOperator live; Vonage RS256 JWT from tenant private key (`_shared/vonage_jwt.ts`). **Telnyx:** dial with `record=record-from-answer` + webhook; on `call.answered` webhook speaks queued script via Call Control `speak`; `call.recording.saved` → `bos-voice-complete` (Sarvam STT). Settings: masked saved keys + Test dial + **Verify keys** (`bos-voice-verify`) + **webhook URL copy** (Twilio/Exotel/Telnyx) + **Sarvam TTS preview** (`bos-voice-tts`). Webhooks `bos-voice-webhook` also handle inbound/missed → find/create lead. Soft-delete via `bos_voice_calls.deleted_at`. Sales/campaigns use tenant voice provider. Reports: voice live/stub/STT/inbound/avg duration.
