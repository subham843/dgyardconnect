...
**Voice multi-provider:** Tenant secrets under `api_secrets.voice.<provider>` (`exotel` | `twilio` | `plivo` | `vonage` | `knowlarity` | `myoperator`). Active provider in `api_config.voice.provider`. `bos-voice-dial`: Exotel/Twilio/Plivo/Knowlarity/MyOperator live; Vonage RS256 JWT from tenant private key (`_shared/vonage_jwt.ts`). Settings: masked saved keys + Test dial. `bos-voice-complete`: Sarvam STT when `api_secrets.sarvam` / `SARVAM_API_KEY` + `audio_url`/`recording_url`.
